...
//  动态添加导航菜单
    $.ajax({
        ...
        success: function(data) {
            ...
            for (key in data) {
                var liHtml = "";
                childListLength = data[key].childList.length;
                for (var i = 0; i < childListLength; i++) {
                    liHtml += "<li><a href='" + data[key].childList[i].protocol + "://" + data[key].childList[i].servernm + data[key].childList[i].subsyproject + data[key].childList[i].scope + "'>" + data[key].childList[i].des + "</a></li>";
                }
                navIndexarray.push("<li class='login-pa-li' id=' " + data[key].id + "'>" + data[key].des + "<ul class='login-ch-ul'>" + liHtml + "</ul></li>");
            }
...
//      帮助和采购政策详情
            var detailshelppolicy = function(parentClass,childClass,detailClass) {
                $("." + parentClass).on("click","." + childClass,function() {
                    ...
                    $("." + childClass).find("h4").text(data[thisId].title);
                    $("." + childClass).find(".name").text(data[thisId].name);
                    $("." + childClass).find(".date").text(data[thisId].date);
                    $("." + childClass).find(".modalbody-content").html(data[thisId].content);
                    var filelinkhtml; //单个附件链接
                    var filelinkhtmlArray = []; //附件链接集合
                    var finalfileshtml;  //所有链接链接
                    if(data[thisId].filelink) {
                        for(var i = 0;i < data[thisId].filelink.length;i++) {
                            filelinkhtml = "<a href='" + data[thisId].filelink[i] + "' target='blank'>" + data[thisId].filename[i] + "</a>";
...
//    采购政策分页点击事件
            clickhelppolicpages("showpolicymodal","showpolicymodal",data["policy"],"showpolicymodal");
