...
public class Register {
     ...
     * @param mapResponse 
     */
    public void referto_submint(Map mapRequest, Map mapResponse){
        //供应商基本信息
        ...
        try {
            String sup_reg_id = supRegService.getSupRegID();//获取CUX_SRM_SUP_REG表ID
            long id = Long.valueOf(sup_reg_id);
            ...
            //设置经营性质各个字段值初始为'N'
            supReg.setManufacturers_supplier("N");
            supReg.setAgent_supplier("N");
            supReg.setOutsourcingcompany_supplier("N");
            supReg.setRetailer_supplier("N");
            supReg.setServiceprovider_supplier("N");
            supReg.setLogisticscarriers_supplier("N");
            supReg.setReclaimingagents_supplier("N");
            //经营性质:选中为'Y',未选中为'N'
...
//            }
            ...
            supReg.setCreated_by(6);
            supReg.setCreation_date(new Date());
            supReg.setLast_updated_by(6);
            supReg.setLast_update_date(new Date());
            supReg.setLast_update_login(6);
            //业务联系人信息
            ...
            List<SupplierRegisterInfo> supRegInfoList = new ArrayList<SupplierRegisterInfo>();
            if(contancts_name != null && contancts_name.length > 0){
                int i = contancts_name.length;
                for(int j=0;j<i;j++){
                    SupplierRegisterInfo supreginfo = new SupplierRegisterInfo();
                    supreginfo.setSupregId(id);
                    supreginfo.setRegistered_comapny_name(registered_comapny_name);
                    supreginfo.setContancts_name(contancts_name[j]);
                    supreginfo.setContancts_position(contancts_position[j]);
                    supreginfo.setContancts_office_phone(contancts_office_phone[j]);
                    supreginfo.setContancts_email(contancts_email[j].trim());
                    supreginfo.setContancts_phone(contancts_phone[j]);
                    supRegInfoList.add(supreginfo);
                }
            }
            Map map = new HashMap();
            map.put("p_reg_id", sup_reg_id);
            map.put("p_error", "");
            map.put("p_error_msg", "");
            boolean flag = supRegService.insertSupplierRegister(supReg, supRegInfoList);//供应商注册信息存储到表中
            if(flag){
                boolean msg = supRegService.supplier_register(map);//供应商信息注册
                if(msg){
                    ...
                    if("S".equals(p_error)){
                        ...
                        AjaxResponse.writeOutString("<script>window.location.href='../supplierregister/SupplierModify.jsp?id="+id+"';</script>");//注册失败，跳转到供应商更改页面
                    }
                }else{
                    AjaxResponse.writeOutString("<script>window.location.href='../supplierregister/SupplierModify.jsp?id="+id+"';</script>");//调用注册接口失败跳转页面
                }
            }else{
                throw new Exception(LanguageNational.getValue("msg00003"));//供应商信息存储失败
            }
