    ...
    <body>
        <div class="wrap">
            ...
            <c:forEach items="${zbHeaders}" var="zbHeaders">
                <div class="showannoumenlist_${zbHeaders.zb_header_id} modalvisble provide-modal providesignup-modal addClassfile">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-body pubannou-body">       
                                <div class="purchas">
                                    ...
                                    <div class="closeingdate">
                                        【报名截止时间：<span class="closedate"><fmt:formatDate value='${zbHeaders.zb_enrol_end_date}' pattern='yyyy-MM-dd HH:mm' /></span>】
                                    </div> 
                                </div>
                                    ...
                                    <table class="table table-striped table-hover table-bordered">                              
                                        <tbody>
                                            <tr>
                                                ...
                                                <td style="display: none">${zbHeaders.zb_header_id}</td>
                                            ...
                                            <tr>
                                                ...
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_answer_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                            </tr>
                                            <tr>
                                                <td class="first-child">购买招标文件时间</td>
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_project_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                                <td class="first-child">投标开始时间</td>
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_bid_start_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                            </tr>
                                            <tr>
                                                <td class="first-child">购买招标文件地点</td>
                                                <td class="last-child-td">${zbHeaders.zb_project_address}</td>
                                                <td class="first-child">投标截止时间</td>
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_bid_end_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                            </tr>
                                            <tr>
                                                <td class="first-child">招标文件售价</td>
                                                <td class="last-child-td">${zbHeaders.zb_flie_je}</td>
                                                <td class="first-child">开标时间</td>
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_open_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                            </tr>
                                            <tr>
                                                <td class="first-child">投标保证金</td>
                                                <td class="last-child-td">${zbHeaders.zb_vendor_bond}</td>
                                                <td class="first-child">申诉时间</td>
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_appeal_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                            </tr>
                                            <tr>
                                                <td class="first-child">法人名称</td>
                                                <td class="last-child-td">${zbHeaders.zb_corporation}</td>
                                                <td class="first-child">报名截止时间</td>
                                                <td class="last-child-td"><fmt:formatDate value='${zbHeaders.zb_enrol_end_date}' pattern='yyyy-MM-dd HH:mm' /></td>
                                            </tr>
                                            <tr>
                            ...
                            <div class="modal-footer purchas-footer">
                                <div class="agree">
                                    <input type="checkbox" id="agreefor"/><label for="agreefor">我司同意遵守 <a href="javascript:void(0);" class="showmoralconductmodal showmodala text-primary clickmoral">《比亚迪供应商道德行为规范》</label></a>
                                </div>
                                <button type="button" class="btn btn-warning singupbtn btn-sm" data-toggle="modal" data-dropback="static" id="singup">点击参加报名</button>
				<form id='' method='post' class='sinupsform removeform'>
        ...
        <script>
            ...
            $(".addform").on("click",".singupbtns",function(){
                ...
                if($("." + thisId).valid()){
                    focusCleanup:true;
                    $(this).parents("." + thisId).attr("action", "../SrmHome/SupplierSignup.jsp?__METHOD=submit_signup");
            ...
            $(function() {
//                采购招标公告详情窗口添加附件显示
                var thisClass = $(".providesignup-modal").attr("class").split(" ")[0];
                ...
                var finalAhtml;
                $.ajax({
                    url: "../SrmHome/Login.jsp?__METHOD=getSrmAttachment",
                    dataType: 'json',
                    type: 'POST',
                    data: $.param({header_id:thisId,filetype:"ZB_FILE"}),
                    success: function(data) {
                        ...
                        $(".providesignup-modal").find(".filedisplay").html(finalAhtml);
                    },
                    error: function() {
                        $(".providesignup-modal").find(".filedisplay").html("无");
                    }
                });
                
                //供应商编码自动带出
                $(".providesignup-modal").on("blur","#description",function() {
                    var thisName = $(this).val();
                    $.ajax({
                       url: "../SrmHome/SupplierSignup.jsp?__METHOD=getSupCode" ,
                       dataType: "json",
                       type: "POST",
                       data: $.param({vendor_name:thisName,ts_header_id:thisId}),
                       success: function(data) {
                           $("#segment1").val(data.segment1);
                           if (data.isNot) {
                               //表示已经报名过此项目
                               $("#description").after("<div class='repeatname'>该供应商已报过名，无法再次报名,请修改</div>");
                               $(".providesignup-modal").find(".singupbtns").prop("disabled",true);
                           } else {
                               $("#description").next(".repeatname").remove();
                               $(".providesignup-modal").find(".singupbtns").prop("disabled",false);
                           }
                       }
                    });
                });
                $(".providesignup-modal").on("keydown","#description",function() {
                    $("#segment1").val(" ");
                    $("#description").next(".repeatname").remove();
                    $(".providesignup-modal").find(".singupbtns").prop("disabled",false);
                });
                ...
                //招标公告报名截止时间
                $(".providesignup-modal").each(function(){
                    var textTime = $(this).find(".closedate").text();
                    var needTime = textTime.replace(/-/g,"/");
                    var thisDate = new Date(needTime);
                    var nowDate = new Date();
                    var thisDatestr = Date.parse(thisDate);
                    var nowDatestr = Date.parse(nowDate);
                    if(nowDatestr > thisDatestr) {
                        $(this).find(".purchas-footer").html("<div class='agree'>抱歉，已过了报名截止时间，请关注其他招标公告，谢谢您的关注！</div>");
                    }
                });
            });
        </script>
